...
export default function InsightsScreen() {
  ...
  const processedData = useMemo(() => {
    ...
    if (range === 'year') {
      rangeTxs = allTransactions.filter(t => new Date(t.date).getFullYear() === selectedYear);
    } else if (range === 'month') {
      // ✅ CORRECTED LOGIC: Filter by Current Month AND Selected Year
      rangeTxs = allTransactions.filter(t => {
        const d = new Date(t.date);
        return d.getMonth() === now.getMonth() && d.getFullYear() === selectedYear;
      });
    } else {
      const rangeStartDate = getRangeDates('week');
      rangeTxs = allTransactions.filter(t => new Date(t.date) >= rangeStartDate);
    }
    ...
    if (range === 'week') {
      ...
      for (let i = 1; i <= daysInMonth; i++) chartMap[i] = { id: i, label: String(i), fullLabel: `${i} ${now.toLocaleString('default', { month: 'short' })}`, value: 0 };
    } else {
  ...
  // --- 🖨️ PDF GENERATION HANDLER (ROBUST & ANDROID COMPATIBLE) ---
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      // ✅ CHECK PLATFORM FOR PROPER FILE HANDLING
      let shareUri = uri;
      if (Platform.OS === 'android') {
        try {
          // On Android, copy to a readable cache directory or document directory
          const pdfName = `InSpend_Report_${Date.now()}.pdf`;
          const newUri = FileSystem.documentDirectory + pdfName;
          await FileSystem.copyAsync({ from: uri, to: newUri });
          shareUri = newUri;
        } catch (e) {
          console.log("Failed to copy PDF, sharing original URI:", e);
  ...
  const sendChatMessage = async (isInitial = false) => {
    ...
    if (isInitial) {
      ...
      if (!valResult.success) {
        Alert.alert("Input Error", valResult.error.issues[0].message);
        return;
      }
  ...
  const handleRangeSelect = (r: "week" | "month" | "year") => {
    setRange(r);
  ...
  // ✅ DYNAMIC LABEL (UPDATED)
  const dateLabel = useMemo(() => {
    if (selectedLabelText) {
      if (range === 'month') return `${selectedLabelText.toUpperCase()} ${selectedYear}`;
      return `SELECTED: ${selectedLabelText.toUpperCase()}`;
    }
    if (range === 'year') return `${selectedYear}`;

    // ✅ LOGIC FIX: Show Selected Year
    if (range === 'month') {
      return `${new Date().toLocaleString('default', { month: 'short' }).toUpperCase()} ${selectedYear}`;
    }
    return 'THIS WEEK';
  }, [range, selectedYear, selectedLabelText]);
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.bg }]}>
      ...
      >
        <View style={styles.header}>
          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
            ...
            {!loading && allTransactions.length > 0 && (
              <TouchableOpacity
                ...
                disabled={pdfLoading}
                activeOpacity={0.85}
                style={{ marginLeft: 10, overflow: 'hidden', borderRadius: 14 }}
              >
                <Animated.View
                  style={{
                ...
                >
                  {pdfLoading ? (
                    <>
                      ...
                      <Text style={{ fontSize: 11, fontWeight: '800', color: GOLD_COLOR, letterSpacing: 0.8, textTransform: 'uppercase' }}>Generating</Text>
                    </>
                  ) : (
                    <>
                      <Ionicons name="sparkles" size={14} color={GOLD_COLOR} style={{ marginRight: 5 }} />
                      <Text style={{ fontSize: 11, fontWeight: '800', color: GOLD_COLOR, letterSpacing: 0.8, textTransform: 'uppercase' }}>Report</Text>
                    </>
                  )}
                </Animated.View>
              </TouchableOpacity>
          ...
          {/* RANGE SELECTOR (Week/Month/Year) - Top Right */}
          <View>
            <TouchableOpacity style={[styles.rangeBtn, { backgroundColor: theme.card }]} onPress={() => setShowRangeDropdown(true)}>
            ...
            <View style={styles.statsRow}>
              <View>
                ...
                >
                  ...
                  {range !== 'week' && <Ionicons name="chevron-down" size={12} color={theme.muted} style={{ marginLeft: 4 }} />}
                </TouchableOpacity>
                ...
                <Modal visible={showYearDropdown} transparent animationType="fade">
                  <TouchableWithoutFeedback onPress={() => setShowYearDropdown(false)}>
                    <View style={styles.modalOverlay}>
                      <View style={[styles.dropdownMenu, { backgroundColor: theme.card, top: 150, left: 30, width: 100 }]}>
                        {/* Show years 2024, 2025, 2026 */}
                        {[2024, 2025, 2026].filter(y => y <= new Date().getFullYear()).map(year => (
                          <TouchableOpacity key={year} style={[styles.dropdownItem, selectedYear === year && { backgroundColor: theme.border }]} onPress={() => handleYearSelect(year)}>
                            <Text style={[styles.dropdownText, { color: theme.text }]}>{year}</Text>
                            {selectedYear === year && <Ionicons name="checkmark" size={14} color={theme.text} />}
                          </TouchableOpacity>
                        ))}
                      </View>
                    </View>

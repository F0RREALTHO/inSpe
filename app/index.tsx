...
import { useData } from "./context/DataProvider"; // ✅ Import Data Hook

export default function LoginScreen() {
  const router = useRouter();
  const { refreshData } = useData(); // ✅ Get refresh function
    ...
    try {
      ...
      if (error.code === 'auth/invalid-credential' || error.code === 'auth/user-not-found' || error.code === 'auth/wrong-password') {
        try {
          ...
          refreshData(); // ✅ Fetch empty data structure for new user
  ...
  const handleGoogleLogin = () => {
    Alert.alert("Development Mode", "Google Login requires a development build.");
  };

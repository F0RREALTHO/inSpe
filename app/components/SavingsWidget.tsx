...
import { doc, updateDoc, arrayUnion, arrayRemove, writeBatch, collection } from 'firebase/firestore';
...
export default function SavingsWidget() {
  ...
  // Active = Not Locked AND (No Target OR Target Not Reached)
  const activeGoals = allGoals.filter(g => !g.isLocked && (g.target === 0 || g.current < g.target));
  // Completed = Locked OR Target Reached
  ...
  const safeToSpend = useMemo(() => {
      ...
      return Math.min(budgetRemaining, balance); // Allow negative for detection
  ...
  const handleLockGoal = async () => {
      ...
      Alert.alert(
          ...
          [
              ...
              // 1. CASH: Unlock + Spend Instantly (Visible in Insights)
              { 
                  text: "💵 Cash / No SMS", 
                  onPress: async () => {
                      setLoading(true);
                      try {
                          ...
                          // A. UNLOCK FUNDS (Income to Safe-to-Spend)
                          ...
                          // B. RECORD EXPENSE (Real Purchase for Insights)
              ...
              // 2. CARD: Unlock Only (SMS will handle expense)
              { 
                  text: "💳 Card / UPI", 
                  style: 'default',
                  onPress: async () => {
  ...
  return (
    <View style={{ marginTop: 30, zIndex: 10, elevation: 10, position: 'relative' }}> 
      ...
      {/* 🚨 BUDGET RESCUE BANNER (Scenario C) */}
      ...
      >
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.modalOverlay}>
            <View style={[styles.premiumModal, { backgroundColor: theme.card }]}>
                ...
                {/* ✅ EARLY PURCHASE BUTTON (ANY ACTIVE GOAL WITH MONEY) */}
